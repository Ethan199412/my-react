...
class ReactNativeUnit extends Unit {
    ...
    for (let propName in props) {
      ...
      // 如果是子元素
        ...
        contentStr = (props["children"] as Element[])
          ...
          .join("");
        console.log(
          "[p3.0] 改变 _renderedChildrenUnits",
          this._renderedChildrenUnits
        );
      }
      // 如果是其他属性
  ...
  patch(diffQueue: IDiff[]) {
    ...
    for (let i = 0; i < diffQueue.length; i++) {
      ...
      if (
        ...
        const { fromIndex, unit } = difference;
    ...
    for (let i = 0; i < diffQueue.length; i++) {
      ...
      const { type, unit, toIndex } = difference;
  ...
  diff(diffQueue: IDiff[], newChildrenElements: Element[]) {
    ...
    console.log("[p1.1]", {
      renderUnits: this._renderedChildrenUnits,
      oldChildrenUnitMap,
      newChildrenUnitMap,
    });
  ...
  updateDOMProperties(
    ...
    for (propName in newProps) {
      ...
      if (propName == "children") {
        console.log("[p3.1]", newProps[propName]);
        continue;
      }
      // 如果是事件处理函数
